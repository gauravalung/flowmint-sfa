...
## 2026-09-10 — Slice C built: online ordering (offline outbox deferred)

**Context:** By this point Slice B (today's beat, visit check-in/close, off-beat search, new outlet OTP flow) had already been completed in an earlier session (commit `ef45d02`). The user asked to add ordering. Slice C as locked in `SFA_MVP_Scope_Locked.md` bundles catalog + cart + order submit + GST/scheme calc together with an offline outbox (local queue + retry when connectivity drops) — a meaningfully separate, heavier piece of work than the rest combined.

**Decision (via AskUserQuestion):** build online ordering first — catalog, cart, order submit with the real GST + scheme discount calc, server-side idempotency via `client_uuid` — deferring the offline outbox as a clearly-flagged follow-up rather than silently dropping it from scope. Gets a working, testable order flow fastest; the DB schema (`products`, `sales_orders`, `sales_order_items`) and the shared contract (`packages/shared`'s `getDiscountPct`, `ProductSummary`, `OrderResult`, `createOrderSchema`, etc.) were already in place from Slice A, so this was "just" the API modules and mobile screens on top of an already-designed schema.

**Built:**
- `packages/shared/src/scheme.ts`: added `computeOrderTotals()` — the full §6 formula (order-level discount apportioned per-line by pretax share, each line's own GST rate applied post-discount, rounded to 2dp at the line level before summing). `getDiscountPct()` itself was already there from an earlier session.
- `apps/api/src/modules/products`: `GET /products` (search + `category_id` + pagination) and `GET /products/categories`, mirroring `retailerRepository.searchRetailers`'s pattern.
- `apps/api/src/modules/orders`: `POST /orders` — validates retailer/visit ownership, derives `beat_id` from the visit (never trusts it from the client), fetches server-authoritative price/GST per product (never client-supplied), computes totals via the shared calc, and persists order + snapshotted line items in one transaction. Idempotent by `client_uuid` (same `ON CONFLICT DO NOTHING` + re-fetch pattern as `visitRepository.createVisit`, extended across the order+items pair). When placed against a visit, that visit is closed as `ORDER_BOOKED` in the *same* transaction as the order insert — either both happen or neither does.
- Mobile: `CartContext` (in-memory only — no persistence, no offline outbox, deliberately deferred per the scope decision above), `ProductCatalogScreen` (search + category chips + paginated virtualized list + quantity steppers), `CartScreen` (live total preview using the same shared calc, `POST /orders` on submit). `RetailerDetailScreen`'s "Start Visit" now lands back on itself in the `IN_PROGRESS` branch (previously it jumped straight to `CloseVisit`, since there was nothing else to do) with a new "Take Order" button alongside the existing "Close Visit — No Order" path.

**Verified, not just written** (real local Postgres in this sandbox — a system cluster was available, unlike the Docker unreachability noted in the Slice A entry): full order flow via curl against the live API — login → start visit → browse/search catalog + categories → place a 2-line order → hand-verified the returned subtotal/discount/GST/grand-total against the §6 formula by hand (₹3,680 subtotal → 2% slab → ₹4,255.55 grand total, matched exactly) → confirmed the visit auto-closed as `ORDER_BOOKED` → retried the identical request with the same `client_uuid` and got back the same order id, not a duplicate → confirmed a second order against the now-closed visit correctly fails `409 VISIT_ALREADY_CLOSED` → confirmed an unknown product id correctly fails `400 PRODUCT_NOT_FOUND`. Both `apps/api` and `apps/mobile` type-check cleanly; mobile bundles successfully for web (`expo export --platform web`).

**Known gap, stated plainly:** no offline outbox — order submission requires live connectivity, unlike visit check-in/close and catalog browsing which the original spec intended to be offline-tolerant. Cart state is in-memory only (lost if the app is killed mid-order). Both are deferred, not abandoned — `SFA_MVP_Scope_Locked.md` §12 reflects this as Slice C's remaining unbuilt piece.

**Next:** offline outbox (if wanted), then Slice D (order history, same-day cancel, full salesman-day UAT walkthrough).

---

## 2026-09-10 (round 2) — Multi-distributor salesmen: reopens a "not building" decision

**Context:** User asked for a "distributor list → beats → retailers" browse flow. This didn't match how the app is modeled — one employee, one fixed `distributor_id` — and directly touches the "sub-distributors" line logged as explicitly not-building in `SFA_MVP_Scope_Locked.md` §10. Rather than guess at what was meant, asked two rounds of AskUserQuestion before touching anything: first, whether this was really about multiple distributors per salesman versus a separate admin/reporting view; second, once "multiple distributors" was confirmed, whether one salesman needs to actually *work across* several (switching mid-day) versus just browsing a static hierarchy.

**Decision (via AskUserQuestion, both rounds):** yes — one salesman, many distributors, switchable. Not a distributor-of-a-distributor hierarchy (that's what "sub-distributors" in §10 actually meant, and stays out of scope), not an admin-only read view.

**Built:** full writeup in `SFA_MVP_Scope_Locked.md` §15 — new `employee_distributor_mapping` table (many-to-many, backfilled from the existing single `distributor_id` so already-provisioned salesmen keep working unchanged), every distributor-scoped endpoint now takes an explicit `distributorId` validated against that mapping instead of trusting a fixed column, new `GET /distributors` / `GET /me/distributors/:id/beats` / `GET /beats/:id/retailers` endpoints, and a mobile `DistributorContext` + picker that's invisible for the common single-distributor case.

**A real constraint surfaced during the build, not anticipated when asked:** `beat_employee_mapping.UNIQUE(employee_id, day_of_week)` means "today's beat" is inherently one beat, period — not one beat per distributor. Resolved by keeping "today's beat" exactly as it was (whichever distributor that one beat happens to belong to) and making the new "browse all beats" list distributor-scoped but *not* day-of-week-gated — it shows every beat under a distributor the employee has access to, matching how off-beat retailer search already works (whole distributor's book, not just beat-assigned retailers). This was a unilateral technical call, not asked as a question — same reasoning as the Prisma swap in the first DECISIONS.md entry: no reasonable alternative existed within the existing schema's constraint, and it changes an implementation detail, not the business behavior that was actually confirmed.

**Verified, not just written:** real local Postgres, curl through the whole hierarchy for two seeded distributors, and specifically the security-critical case — an order claiming the wrong `distributorId` for a retailer that actually belongs to a different one correctly 404s rather than silently succeeding. Both apps type-check; mobile bundles for web. Seed data now includes a second distributor with its own beat/retailers, deliberately left out of `beat_employee_mapping` (no free weekday slots) specifically so this is reachable only through the new browse screens — otherwise the feature would have shipped untestable.
