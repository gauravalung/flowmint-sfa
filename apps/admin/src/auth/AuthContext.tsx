import React, { createContext, useCallback, useContext, useEffect, useMemo, useState } from "react";
import type { EmployeePublic, LoginResponse } from "@flowmint/shared";
import { api, ApiRequestError } from "../api/client";
import * as tokenStore from "./tokenStore";

interface AuthState {
  isLoading: boolean;
  employee: EmployeePublic | null;
  accessToken: string | null;
  login: (employeeCode: string, password: string) => Promise<void>;
  logout: () => void;
}

const AuthContext = createContext<AuthState | undefined>(undefined);

const EMPLOYEE_STORAGE_KEY = "flowmint_admin.employee";

export function AuthProvider({ children }: { children: React.ReactNode }) {
  const [isLoading, setIsLoading] = useState(true);
  const [employee, setEmployee] = useState<EmployeePublic | null>(null);
  const [accessToken, setAccessToken] = useState<string | null>(null);

  useEffect(() => {
    tokenStore.hydrate();
    setAccessToken(tokenStore.getAccessToken());
    const stored = localStorage.getItem(EMPLOYEE_STORAGE_KEY);
    if (stored) {
      try {
        setEmployee(JSON.parse(stored) as EmployeePublic);
      } catch {
        // ignore malformed stored value
      }
    }
    setIsLoading(false);

    return tokenStore.subscribe((state) => {
      setAccessToken(state.accessToken);
      if (!state.accessToken) {
        setEmployee(null);
        localStorage.removeItem(EMPLOYEE_STORAGE_KEY);
      }
    });
  }, []);

  const login = useCallback(async (employeeCode: string, password: string) => {
    const result = await api.post<LoginResponse>("/auth/login", { employeeCode, password });
    if (result.employee.role !== "ADMIN") {
      throw new ApiRequestError(403, "NOT_ADMIN", "This portal is for ADMIN accounts only.");
    }
    tokenStore.setTokens(result);
    setEmployee(result.employee);
    localStorage.setItem(EMPLOYEE_STORAGE_KEY, JSON.stringify(result.employee));
  }, []);

  const logout = useCallback(() => {
    tokenStore.clear();
    setEmployee(null);
    localStorage.removeItem(EMPLOYEE_STORAGE_KEY);
  }, []);

  const value = useMemo(
    () => ({ isLoading, employee, accessToken, login, logout }),
    [isLoading, employee, accessToken, login, logout]
  );

  return <AuthContext.Provider value={value}>{children}</AuthContext.Provider>;
}

export function useAuth(): AuthState {
  const ctx = useContext(AuthContext);
  if (!ctx) throw new Error("useAuth must be used within AuthProvider");
  return ctx;
}

export { ApiRequestError };
